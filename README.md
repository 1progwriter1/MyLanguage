...
Во время первого измерения виртуальный процессор оказался медленнее в 15000 раз (!). Этот результат мне показался неправдоподобным. Чтобы понять причину такой "колоссальной" разницы, было принято решение получить временной профиль программы при помощи связки `callgrind + valgrind: `
```sh
...
Большую часть времени занимает функция расчета хеша для верификации стека. Уберем эту функцию. Помимо этого отключим `assert()`, отключим верификацию и скомпилируем с флагом оптимизации `-Ofast`.
...
Разница примерно 64,8 раза. Получим еще раз временной профиль программы, чтобы удостовериться, что больше ничего не замедляет работу процессора.

<figure>
<img src="images/fast.png" width=800>
</figure>

Влияют на время работы только основные функции.

### Итоговый результат: Программа, скомпилированная под `intel_x86_64` быстрее в 64,8 раза.